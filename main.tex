...
\begin{itemize}
	\item A \textbf{controller} manages the execution of functions; it is event-driven and triggered by events created consequently to users' actions, such as REST-ful HTTP queries or new data added to a storage driver.
	\item A \textbf{database} contains the users' functions and the output of completed executions.
...
Serverless computing is a broad and complex topic that covers many subtopics, such as performance, security or caching. 

While some publications focus on a specific subtopic, reviewing the latest academic literature in that area, this work firstly takes a broader approach by analyzing surveys and articles to offer a comprehensive overview of the topic from multiple perspectives.

Then, it explores innovative and notable studies addressing individual challenges, ultimately providing a holistic perspective on serverless computing.

\section{Performance}
\label{sec:performance}
As mentioned above, the execution of functions is performed in isolate sandboxes. Based on the status of the containers, two types of startups are defined:
...
Another advantage of serverless computing included in the bill is its cost model, a pay-as-you-go pricing where the end-users pay only based on the actual resources used, and while the expense of idle resources is covered by the cloud provider, as depicted in Figure \ref{fig:serverlesscost}.
...
Even with dynamic resource allocation, serverful computing operates on a reservation model, requiring computing resources to be allocated based on the application's peak usage. As Castro et al. point out \cite{schleier-smith_what_2021}, in practice, customer realize saving when moving their applications to serverless, but the price reductions and the increased functionalities offered by the cloud providers could threaten their revenues. 
